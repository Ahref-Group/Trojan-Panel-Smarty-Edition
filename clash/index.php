...
function listAllAvailableTrojanNodeNameByToken($token)
{
    $availableNodeArray = listAvailableByUserLevel(getUserProfileByToken($token)['plan']);
    $text = NULL;
    $count = count($availableNodeArray);
    for($i = 0; $i < $count; $i++) {
        $text = $text."    - '".$availableNodeArray[$i]['node_name']."'".PHP_EOL;
    }
    return $text;
}

function listAllAvailableTrojanURLByToken($token)
{
    $availableNodeArray = listAvailableByUserLevel(getUserProfileByToken($token)['plan']);
    $text = NULL;
    $count = count($availableNodeArray);
    for($i = 0; $i < $count; $i++) {
        $text = $text.'  # trojan'.PHP_EOL.'  - name: "'.$availableNodeArray[$i]['node_name'].'"'.PHP_EOL.'    type: trojan'.PHP_EOL.'    server: '.$availableNodeArray[$i]['node_server'].PHP_EOL.'    port: '.$availableNodeArray[$i]['port'].PHP_EOL.'    password: '.$_GET['token'].PHP_EOL.'    udp: true'.PHP_EOL.PHP_EOL;
    }
    return $text;
}
...
function getNodeInfoByLevel($level) {
    $sql = 'SELECT * FROM ss_node where node_type = ' . $level;
    return DBQuery($sql)->fetch_all(MYSQLI_ASSOC);
}

function getUserProfileByUid($uid) {
    $sql = 'SELECT * FROM users where uid = ' . $uid;
    return DBQuery($sql)->fetch_assoc();
}

function getUserProfileByToken($token) {
    $sql = "SELECT * FROM users WHERE display_pwd = '$token'";
    return DBQuery($sql)->fetch_assoc();
}

function DBQuery($sql) {
    @$mysql = new mysqli(DB_HOST, DB_USER, DB_PWD, DB_DBNAME, (int)DB_PORT);
