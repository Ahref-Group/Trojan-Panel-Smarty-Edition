...
if($c->IsEmailUsed($email)){
    ...
    if($rst->IsAbleToReset()){
       ...
       //邮件内容
       $Mail_content = '请访问此链接：<a href="'.$site_url."auth/resetpwd_do.php?code=".$code."&uid=".$uid.'">申请重置密码</a><br />如果不能打开，请复制下面网址到浏览器的地址栏粘贴并按回车！<br />'.$site_url.'user/resetpwd_do.php?code='.$code.'&uid='.$uid;
